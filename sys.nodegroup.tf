module "sys_eks_managed_node_group" {
  ...
  version = "v19.0.3"
  create  = var.create && var.create_component_sys  
  ...
  vpc_security_group_ids = [
    ...
    aws_security_group.node_group_sys[0].id,
  ]
  ...
  labels = var.ng_config_sys.labels

  taints = var.ng_config_sys.taints

  # We use 3 nodes to support the critical pods across the whole cluster.
  min_size       = 3
  max_size       = 3
  desired_size   = 3
  instance_types = var.ng_config_sys.instance_types
